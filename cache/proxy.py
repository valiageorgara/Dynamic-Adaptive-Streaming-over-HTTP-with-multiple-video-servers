...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def request_and_respond(self, segment, address, port):
        ...
        global global_rate_1
        global global_rate_2
        global global_buffer_size
        ...
        try:
            ...
            if r.status >= 400:
                ...
                with open(file_path, 'wb+') as f:
                    ...
                    logging.info('[%s] %s CACHE MISS\n', datetime.now(), segment)
                    logs.append('INFO:root:[' + str(datetime.now()) + '] ' + segment + ' CACHE MISS\n')
                    if 'audio' not in segment and '_61.m4s' in segment:
        ...
        if os.access(file_path, os.R_OK):
            try:
                with open(file_path, 'rb') as f:
                    ...
                    # ging.info('Received and forwarded %s bytes\n', l)
                    logging.info('[%s] %s CACHE HIT\n', datetime.now(), segment)
                    logs.append('INFO:root:[' + str(datetime.now()) + '] ' + segment + ' CACHE HIT\n')
...
def run_server(address="0.0.0.0", port=8000, serve_path=None, server_class=HTTPServer,
    ...
    while next_attempts > 0:
        try:
            ...
            if httpd:
                ...
                httpd.server_close()
                with open('output_' + str(global_rate_1) + '_' + str(global_rate_2) + '_' + str(global_buffer_size) + '_' + str(global_sample) + '.txt', 'w') as the_file:
                    for k in logs:
                        the_file.write(k + '\n')
                break
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r1", "--rate1", help="data rate1", default=100, type=float)
    parser.add_argument("-r2", "--rate2", help="data rate2", default=100, type=float)
    ...
    parser.add_argument("-l", "--buffer_size", help="buffer_size", default="None", type=int)
    parser.add_argument("-sample", "--sample", help="sample", default="None", type=int)
    ...
    mean_rate = args.rate1 * 1024 * 1024 / 8
    mean_rate2 = args.rate2 * 1024 * 1024 / 8
    global_rate_1 = args.rate1
    global_rate_2 = args.rate2
